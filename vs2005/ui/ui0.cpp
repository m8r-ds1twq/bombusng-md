...
int sizecaps;
char **strokicaps;
#define TIMER_TIME 2000 //¬–≈ћя ќѕ–ќ—ј “ј…ћ≈–ј
...
const int IBUFFER_LENGTH = 21504;
const TCHAR pcszAppCaption[] = TEXT("WinInet Test");
...
std::string result;
   ...
   // if (inEsc) throw std::exception("Malformed XML escape sequence");
	Log::getInstance()->msg("news: ",result.c_str());
                    ...
                    {
                        MessageBox(NULL, TEXT("NEWS cannot be read"), pcszAppCaption, 0);
                    }
                }
                else
                {
                    MessageBox(NULL, TEXT("Unable to send request"), pcszAppCaption, 0);
                }
                InternetCloseHandle(hRequest);
            }
            else
            {
                MessageBox(NULL, TEXT("Unable to open http request"), pcszAppCaption, 0);
            }
            InternetCloseHandle(hConnection);
		}
        else
        {
            MessageBox(NULL, TEXT("Unable to open internet connection"), pcszAppCaption, 0);
        }
    ...
    {
        MessageBox(NULL, TEXT("Cannot open Internet"), pcszAppCaption, 0);
    }
}
...
//MessageBox(mainWnd,  TEXT("ѕотер€ св€зи перезапустите программу"),TEXT("ќшибка "), 0);
